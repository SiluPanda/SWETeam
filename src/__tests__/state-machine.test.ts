...
import { validateTransition, transition } from '../session/state-machine.js';
...
describe('session/state-machine — transition', () => {
  ...
  it('should throw for non-existent session', () => {
    expect(() => transition('nonexistent', 'building')).toThrow('Session not found');
  });

  it('should throw for invalid transition', () => {
    expect(() => transition('test-session-1', 'awaiting_feedback')).toThrow('Invalid transition');
  });
});
